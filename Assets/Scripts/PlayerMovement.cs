        ...
        else
        {
            if (!dashstep)
            {
                if (movement.magnitude > 0)
                {
                    ...
                    {
                        MoveCharacter(movement, speed/2);
                    }
            ...
            {
                rb.AddForce(targetDir.normalized * speed * 1.5f * (1 + lastSpeed/3));
            }
            else if (animator.GetCurrentAnimatorStateInfo(0).IsName("Step"))
            {
                rb.AddForce(targetDir.normalized * speed * (1 + lastSpeed / 3));
            }
